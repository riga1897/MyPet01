...
## 5.1 Стратегия раздачи медиафайлов

**Текущий подход:** `ProtectedMediaView → FileResponse` через Nginx — достаточно для приватного семейного сайта.

| Технология | Решение | Причина |
|-----------|---------|---------|
| **S3 для бэкапов** | ✅ Реализовать (шаг 5) | Правило 3-2-1, off-site защита |
| **S3 для медиа** | ⏳ По необходимости | Когда диск VPS1 начнёт заканчиваться |
| **CDN** | ⏳ Автоматически с S3 | Cloudflare R2 включает CDN бесплатно |
| **HLS / DASH** | ❌ Не планируется | Оверкил для 5–20 пользователей; недели FFmpeg-транскодинга ради нулевого выигрыша |

**Принцип:** не усложнять то, что уже работает. Пересматривать только при реальной потребности.
