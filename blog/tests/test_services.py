...
class TestGenerateThumbnailFromVideo:
    ...
    @patch('blog.services.subprocess.run')
    def test_converts_rgba_image_to_rgb(self, mock_run: MagicMock) -> None:
        """Should convert RGBA/P mode images to RGB."""
    ...
    @patch('blog.services.subprocess.run')
    def test_cleans_up_empty_tmp_file(self, mock_run: MagicMock) -> None:
        """Should cleanup tmp file when it exists but is empty."""
        with tempfile.NamedTemporaryFile(suffix='.jpg', delete=False) as tmp:
            tmp_path = tmp.name
    ...
    @patch('blog.services.subprocess.run')
    def test_handles_timeout_exception(self, mock_run: MagicMock) -> None:
        """Should return None and cleanup on timeout."""
        import subprocess
